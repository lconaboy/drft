...
*/
class grafic2_dmm_output_plugin : public output_plugin
{
protected:
  ...
  //float metal_floor_;
	...
	void write_ramses_namelist_old( const grid_hierarchy& gh )
	{
		...
		sprintf(ff,"%s/ramses.nml",fname_.c_str() );
		std::cout	<< " - The grafic2_dmm output plug-in wrote the grid data to a partial\n"
		<< "   RAMSES namelist file \'" << ff << "\'\n"; 
	}
	
public:
	
	grafic2_dmm_output_plugin( config_file& cf )
	: output_plugin( cf )
	{
	...
	{	
	  if(! bhavehydro_ ){
		...
		for(unsigned ilevel=levelmin_; ilevel<=levelmax_; ++ilevel )
		{
			...
			char ff[256];
			sprintf(ff,"%s/level_%03d/ic_deltac",fname_.c_str(), ilevel );
